...
def refine_dupes_by_partial_hash(dupes_dict, partial_check_size=G.PartialCheckSize, hashfunc=G.HashFunc):
    print("refine_dupes_by_partial_hash: begin")
    ...
    for file_size, flist in dupes_dict.items():
        #print("refine_dupes_by_partial_hash " + str(flist))
    ...
    qprint('\nrefine_dupes_by_partial_hash: Dupes: ', len(dupe_exact))
    return dupe_exact
...
def refine_dupes_by_full_hash(dupes_dict, block_size=G.FullBlockSize, hashfunc=G.HashFunc):
    ...
    fullhashes = {}
    print("refine_dupes_by_full_hash: ")
    for selector, flist in dupes_dict.items():
        print("refine_dupes_by_full_hash: selector %s" % str(selector))
        print("refine_dupes_by_full_hash files: " % flist)
        sz = selector[-1]  # Save size so we can still inform the user of the size
        try:
            for fname in flist:
...
def emit_results(dupes_dict, out_format=G.OutFormat, out_file=G.OutFile):
    ...
    if out_file is not None:
        qprint("emit_results: done " + str(out_file))
...
def output_results(dupes_dict, out_format=G.OutFormat, out_file=G.OutFile):
    ...
    # kiys = [k for k in dupes_dict]
    # kiys.sort(key=lambda x: x[-1])
    f = open('/tmp/dupefiles.dat', 'w')
    print('"Ord","Selector","FullPath"', file=out_file)
    order = 1
    for digest, inode_map in dupes_dict.items():
     #   print ("=====>  Duplicates", file=f)
       # print("==> %s %s" % (digest, inode_map), file=f)
      #  print("Duplicates ############# size " + str(ino))
        for inode_entry   in inode_map:
            finfo = FileInfo(inode_entry)
            print('"%s",%d,%d,"%s"' % (digest, finfo.size(), finfo.mod_time(), str(inode_entry)), file=f)
          #  print("inode entry: "  + str(inode_entry))
        # digest_list = []
        # for inode, inode_list in inode_map.items():sgtr
        #     digest_list.extend(inode_list)
        #     # digest_list.sort()
        # for fname in digest_list:
        #     print('"{0}","{1}"'.format(order, key, fname), file=out_file)
        #     order += 1
    print("wrote dupes to " + str(out_file))
    f.close()
def _main(*argv):
    G.StartPaths = argv
...
if __name__ == '__main__':
    _main("/run/media/jjs/DimonBack/dupes")
    print("done")
